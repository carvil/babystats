...
namespace :fetch_data do
  ...
  desc "Fetch data for head_circumference_percentile"
  task :fetch_head_circumference_percentiles_data, [:url, :gender, :measure] => :environment do |t, args|
    ...
    rows.drop(1).each do |row|
      row = row.map{|e| e.strip}
      puts "importing #{row}"
      HeadCircumferencePercentile.create(:gender => args.gender,
                                         :month => row[headers.index('Month')],
                                         :mean => row[headers.index('M')],
                                         :sample_sd => row[headers.index('S')],
                                         :standard_deviation => row[headers.index('SD')],
